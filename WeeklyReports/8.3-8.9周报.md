...
- linear_regression使用一些二维的散点坐标：
  ...
  - 进行回归训练后，画出拟合后的曲线
    ...
    <img src="..\Figures\Regression\linear_regression.JPG" style="zoom:67%;" />
...
- 第一卷积层：通过零填充（padding=2），步幅为1，设置了**16个1x5x5的卷积核**，使得28x28的图像经过该层后不改变维度，依然是28x28（16通道）；之后经过最大池化，核为2x2，步幅为2，缩小一半，输出为14x14（16通道）的图像

- 第二卷积层：同样通过零填充（padding=2），步幅为1，设置**32个16x5x5的卷积核**，保持输出图像和输入图像的维度一样；之后又经过最大池化，缩小一半，输出为7x7（32通道）的图像

- 全连接层：将7x7x32的输入通过线性变换输出为长度为10的Tensor（10个类别）

- 使用TensorBoard显示训练时loss的变化（smoothing=0.6）

<img src="..\Figures\CNN\training_loss.JPG"  />

- 使用matplotlib库显示测试结果和输入图像的对比（图像为输入的测试集图像，标题显示模型的预测类别），下图中4个输入图像有1个分类错误。

<img src="../Figures/CNN/MNIST_predicted_result.JPG" style="zoom:67%;" />
